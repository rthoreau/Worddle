...
public class TextureAtlasSlicer : EditorWindow {
	...
	bool valueOrMid(string name, XmlNode node, out float rep){
		XmlAttribute value = node.Attributes[name];
		if (value == null) {
			rep = 0.5f;
			return false;
		}
		else {
			rep = float.Parse(value.Value);
			return true;
		}
	}
    ...
    public void OnGUI() {
        ...
        EditorGUI.BeginDisabledGroup(focusedWindow != this);
        {
                                          ...
                                          importer.textureType != TextureImporterType.Default));
            {
				...
				if (subTextures == null || subTextures.Count == 0) {
	...
	public static float currentPivotX = 0.5f;
	public static float currentPivotY = 0.5f;
